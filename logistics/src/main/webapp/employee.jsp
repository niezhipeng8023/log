...
<script>
    function xrsj(url) {
        table.render({
            ...
            , height: 451
            , page: true
            , url: '${pageContext.request.contextPath}/depart/'+url //数据接口
            , cellMinWidth: 120
            , cols: [[ //表头
    ...
    function  update(staffNo) {
        layer.open({
            ...
            , yes: function () {
                $.ajax({
                    type: "post"
                    , url: "${pageContext.request.contextPath}/depart/update"
                    , dataType: 'json'
    ...
    function del(license) {
        $.ajax({
            type: "post"
            , url: "${pageContext.request.contextPath}/depart/del/" + license
            , dataType: 'json'
            , success: function (data) {
                layer.msg(data.msg, {time: 1000});
                if (data.code == 3) {
...
<script>
    ...
    $(function () {
        layui.use(['form', 'layedit', 'laydate'], function(){
            ...
            $.ajax({
                type:"get",
                url:"${pageContext.request.contextPath}/dispatch/select",//请求路径
                dataType: 'json'
            ...
            $.ajax({
                type:"get",
                url:"${pageContext.request.contextPath}/firm/select",//请求路径
                dataType: 'json',
                success:function(data){//请求成功后的事件
                    $.each(data.data,function(index,obj){
